...
*   **[CRITICAL] 상수 금지 (No Hardcoding)**: 모든 매매 제어 값(금액, 비율, 시간, 횟수 등)은 반드시 **웹 대시보드와 연결된 DB 설정값(`get_setting`)**을 호출하여 사용해야 한다. 코드 내에 임의의 상수를 하드코딩하는 것은 엄격히 금지된다.
...
*   **기본 손절(Stop Loss)**: 일반 손절 로직은 시스템에서 완전히 제거되었습니다.
*   **조기 손절 (Early Stop-Loss)**:
    - 조건: 현재 단계가 '분할매수횟수 - 1' (예: 5단계 중 4단계)일 때.
    - 기준: 4차 매수 시 계좌 수익률은 약 **-2%**로 수렴한다고 정의한다. (1:1:2:4 비중 효과)

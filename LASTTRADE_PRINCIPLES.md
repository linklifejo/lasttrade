...
*   **기본 손절(Stop Loss)**: '개별종목손절률'이 3이면 -3% 도달 시 즉시 매도한다.
*   **물타기 완료 후 강제 손절**: 
    - 예: 손절률 1%, 물타기 완료(MAX) 후 상태 -2%인 경우, 추가 하락하여 **-3%**가 되면 물타기 수량이 모두 채워진 상태이므로 즉시 매도하여 리스크를 확정한다.
*   **조기 손절 (Early Stop-Loss)**:
    - MAX 직전 단계(예: 5단계 중 4단계)에서도 손절선(-3%)에 도달하면 즉시 매도한다.
    - 목적: 최종 단계(MAX) 투자금을 보호하고, 불필요한 추가 손실을 방지한다.
*   **재난 손절 (Disaster Stop-Loss)**:
    - 물타기 진행 중이라도 수익률이 **-6%** 이하로 하락하면 강제 청산한다.
    - 목적: 예수금 부족 등으로 추가 매수가 불가능한 상황에서 손실 확대를 방지한다.

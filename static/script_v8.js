        ...
        for (const [key, value] of Object.entries(settings)) {
            ...
            if (el) {
                if (el.tagName === 'SELECT') {
                    ...
                    console.log(`📝 SELECT 필드 설정: ${key} = ${valStr}`);
                    ...
                    if (![...el.options].some(o => o.value === valStr)) {
                        ...
                        opt.textContent = valStr + " (DB)";
                        el.appendChild(opt);
                        console.log(`  ➕ 옵션 추가: ${valStr}`);
                    }

                    // 값 설정 (두 가지 방법 모두 사용)
                    el.value = valStr;
                    const idx = [...el.options].findIndex(o => o.value === valStr);
                    if (idx >= 0) {
                        el.selectedIndex = idx;
                        console.log(`  ✅ 설정 완료: ${el.value} (index: ${idx})`);
                    } else {
                        console.warn(`  ⚠️ 옵션을 찾을 수 없음: ${valStr}`);
                    }
...
function applyPreset(presetId) {
    ...
    if (changes.length > 0) {
        ...
        const message = `✅ ${presetNames[presetId]} 설정 적용 완료!\n\n【변경된 항목: ${changes.length}개】\n${changeList}\n\n⚠️ 반드시 '💾 설정 저장' 버튼을 눌러 저장하세요!`;

        alert(message);  // 다이얼로그로 표시

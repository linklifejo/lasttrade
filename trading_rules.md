...
*   **원칙**: 현재 완성된 대시보드의 **'저소음(No Alert) & 고속 저장'** 상태를 최우선으로 유지합니다. (**보드 손대면 너 죽는다.**)
*   **보고 및 투명성 (Transparency)**: 
    1.  **사전 보고 필**: 어떠한 이유로든 대시보드 코드(`index.html`, `script_final.js` 등)를 수정해야 할 경우, 반드시 수정할 내용과 이유를 사용자에게 **미리 알리고 승인**을 받아야 합니다.
    2.  **사후 보고 필**: 수정이 완료된 후에는 구체적으로 어떤 부분이 바뀌었는지 명확하게 보고해야 합니다.
    3.  **무단 수정 금지**: 사용자 모르게 '슬쩍' 코드를 고치는 행위는 절대 금지합니다.
*   **역할 및 책임**: 
...
### 5.2 최소 범위 수정 및 연관성 엄격 준수 (Minimal Scope & No Unrelated Edits)
*   **원칙**: 작업 요청 시, **해당 문제와 직접적으로 연관된 파일 및 코드**만 수정합니다.
*   **세부 지침**:
    1.  **현상 유지 우선**: 이미 정상적으로 작동하는 기능이나 파일은 사용자 요청 없이 먼저 개선하거나 손대지 않습니다. "잘 되는 건 그냥 둔다"는 원칙을 고수합니다.
    2.  **맥락 외 수정 금지**: Python 로직 수정 요청 시 Dashboard(JS/HTML) 파일 등 상관없는 영역을 무단으로 수정하는 행위를 절대 금지합니다.
    3.  **정밀 타격**: 문제가 발생한 지점만을 정확히 파악하여 최소한의 코드 수정으로 해결하며, 주변 코드를 불필요하게 건드리지 않습니다.

## 요약
이 시스템은 자산의 70% 범위 내에서, 설정된 종목 수만큼 자감을 나누고, 각 종목 안에서도 1:1:2 비율(3분할 시)로 나누어, 예수금이 충분하고 전략 조건(불타기/물타기)이 맞을 때만 체계적으로 매수를 실행하여 리스크를 관리합니다. 또한, 안정화된 대시보드와 정상 작동 중인 모든 시스템 요소는 사용자 요청 없이 임의로 수정하지 않으며, 오직 요청된 범위 내에서만 정밀하게 작업함을 절대 원칙으로 합니다.

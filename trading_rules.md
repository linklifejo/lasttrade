...
### 5.1 대시보드 수정 엄격 금지 (Strict No-Touch Policy)
*   **원칙**: 대시보드(Web UI), 특히 `index.html` 및 `script_final.js`는 현재 사용자에게 최적화된 상태이므로, **사용자의 명시적인 요청이 없는 한 임의로 수정하는 것을 엄격히 금지합니다.**
*   **세부 지침**:
    1.  알림창(Alert/Confirm) 제거 및 최적화된 저소음 UI 상태를 유지해야 합니다.
    2.  디자인 요소를 변경하거나 새로운 기능을 독단적으로 추가하지 않습니다.
    3.  대시보드 수정이 필요한 경우, 반드시 수정 내용과 범위를 사전에 설명하고 승인을 받아야 합니다.

## 요약
이 시스템은 자산의 70% 범위 내에서, 설정된 종목 수만큼 자금을 나누고, 각 종목 안에서도 1:1:2 비율(3분할 시)로 나누어, 예수금이 충분하고 전략 조건(불타기/물타기)이 맞을 때만 체계적으로 매수를 실행하여 리스크를 관리합니다. 또한, 안정화된 대시보드는 사용자 요청 없이 임의로 수정하지 않음을 절대 원칙으로 합니다.
